...
class EnrollKeysController extends AppController
{
   ...
   public function form($id = null)
   {
    ...
    $course = $this->Courses->find('list',['conditions'=>['status'=>2]]);
    $defaultCourses = [];
    if($id):
   	$chk = $this->EnrollKeys->is_exists($id,$this->Auth->user('id'));
    endif;
    $getParentCat = $this->EnrollKeys->find('list');
   	if($id && $chk==0)
   	{
    ...
    if($id)
    {
      ...
      $defaultCourses=$enrollkey['courses'];
  /*    $getcourses = $this->Courses->find('all',['conditions'=>['id in '=>[$enrollkey['courses']]]])->toArray();
    ...
    if($this->request->data)
    {
     ...
     // pr($this->request->data); die;
      ...
      $data['courses'] = implode(',',$data['course_id']);
   ...
   public function index()
   {
    ...
    $list = $this->EnrollKeys->find('all',['conditions'=>['EnrollKeys.addedby'=>$user_id],'contain'=>['Departments','SubDepartments']]);
