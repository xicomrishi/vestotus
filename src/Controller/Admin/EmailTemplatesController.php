...
class EmailTemplatesController extends AppController
{
   ...
   public function form($id = null)
   {
    ...
    if($id)
   	{
   		$template = $this->EmailTemplates->get($id);
    if($this->request->data)
    {
        $this->request->data['addedby'] = $this->Auth->user('id');
        $this->request->data['status'] = 1;
        $template = $this->EmailTemplates->patchEntity($template,$this->request->data);
        if($this->EmailTemplates->save($template))
        {
   ...
   public function index()
   {
    ...
    $list = $this->EmailTemplates->find('all');
