...
class LessonsController extends AppController
{
   ...
   public function view($id=null)
   {
    ...
    if($lesson == 1)
    {
      $lesson = $this->CourseChapters->find('all',['conditions'=>['CourseChapters.id'=>$id,'CourseChapters.type !='=>'assessment'],'contain'=>['Courses']])->last();
   ...
   public function update()
   {
    ...
    if($this->request->isajax())
    {
        ...
        $this->CourseProgress->updatedata($dbup);
    }
   ...
   private function testPermission($id=null)
   {
    ...
    $course = $this->Courses->get($id,['contain'=>['CourseChapters','CourseResources']]);
       ...
       foreach($course['course_chapters'] as $chapters)
       {
        ...
        {
        $get = $this->CourseProgress->find('all',['conditions'=>
            ['course_id'=>$chapters['course_id'],
              'lesson_id'=>$chapters['id'],
              'user_id'=>$user_id,
              'is_completed'=>1
            ]
            ]
   ...
   public function quizresult($courseId = null)
   {
    $this->loadModel('TestResults');
    $this->loadModel('Onlinetests');
    $this->loadModel('Courses');
