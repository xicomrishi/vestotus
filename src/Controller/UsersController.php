...
class UsersController extends AppController {
    ...
    public function login() {
        ...
        if ($this->request->is('post')) {
            ...
            //first try login using account access table, if record not present then check in users table and continue as normal user
            //if record foud in account access table, then get users info manually 
            $users = $this->Auth->identify();

            if(!empty($users)){

                $acc_ass = $this->UserAccountAccesses->findById($users['id'])->first();

                //manage this credentials, can not be used second time
                $acc_ass = $this->UserAccountAccesses->patchEntity($acc_ass, ['used' => date('Y-m-d H:i:s')]);
                $this->UserAccountAccesses->save($acc_ass);

                $users = $this->Users->findById($users['user_id'])->first();
            } else{
                $this->Auth->config(
                    'authenticate', [
                        'Form' => [
